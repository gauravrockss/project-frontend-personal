...
import { useSetOrganizationId } from '../hooks/Authorize';
...
const OrganizationTable = ({ setContent, children, organizations, fetchOgranizations }) => {
    ...
    const acceptInvitation = React.useCallback(
        async function (id, memberId) {
            start();
            try {
                const response = await authServer.post(
                    `/organization/${id}/member/accept/${memberId}`
                );
                const { success, errors } = response.data;
                if (!success) showError(errors);
        ...
        [setContent, children, setOrganizationId, showError, start, end, fetchOgranizations]
    );
